...
internal static class Program {
  ...
  private static readonly OptionSet OptionParser = new() {
    { "b|bundle=", "the original asset {BUNDLE} path",
      v => Options.InputAssetBundlePath = v },
    { "i|input=", "the {INPUT} directory of the assets to overwrite with.",
      v => Options.InputDirectory = v },
    { "o|output=", "the path of the asset bundle you wish to {OUTPUT}.",
      v => Options.OutputAssetBundlePath = v },
    { "d|dump=", "the path of the directory you wish to {DUMP} to.",
      v => Options.DumpPath = v },
    { "m|member=", "the {MEMBER} you dump/overwrite.",
      v => Options.Member = v },
    { "t|texture", "Interact with {TEXTURE}s in the asset bundle.",
      v => Options.Textures = v != null },
    { "h|?|help", "show this message for and then exit.",
      v => Options.Help = v != null }
  };
  ...
  public static void Main(string[] args) {
    ...
    var replace =
        Options.InputDirectory != null && Options.OutputAssetBundlePath != null;

    // ERROR: Not enough information for operations was specified
    if (!(dump || replace)) {
      Console.Error.WriteLine(
          "To DUMP, the BUNDLE, MEMBER, and DUMP arguments must be specified.");
      Console.Error.WriteLine(
    ...
    try { // Textures
      ...
      // ERROR: Not enough arguments provided
        ...
        Console.Error.WriteLine(
            "A MEMBER must be specified or interact with TEXTURES.");
        ShowHelp();
      }
    }
    // ERROR: Could not open the file
    catch (System.IO.IOException) {
      Console.Error.WriteLine("ERROR: Could not open the asset bundle!");
    }
  }
  ...
  private static void RunOperation(Asset uar, bool dump, bool replace) {
    // Dump
    if (dump)
      uar.Dump(Options.DumpPath);

    // Replace
    if (replace)
      uar.Replace(Options.InputDirectory, Options.OutputAssetBundlePath);
  }
